...
function sendMoney(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = mccJs.transaction.createTransaction(
    options.to,
...
function registerDelegate(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = mccJs.delegate.createDelegate(
    options.secret,
    options.username,
    options.secondSecret
  );
...
function setSecondSecret(options) {
  var trs = mccJs.signature.createSignature(options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.success);
  });
}
...
function registerDapp(options) {
  ...
  var trs = mccJs.dapp.createDapp(options.secret, options.secondSecret, dapp);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.success);
